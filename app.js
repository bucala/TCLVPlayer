...
function epgListTemplateText() {
  var lines = state.epgSources
    .filter(function(s) { return s.source && /^https?:\/\//i.test(s.source); })
...
async function importSettingsFromFile(file) {
  try {
    ...
    if (Array.isArray(data.playlists)) { state.playlists = data.playlists; savePlaylistMeta(); if (!state.activePlaylistId && state.playlists.length) state.activePlaylistId = state.playlists[0].id; }
    ...
    // Imported EPG sources arrive with metadata only (no cached .text), and
    // activatePlaylist() only reloads EPG as a side effect of successfully
    // loading playlist text — which a re-imported *local* playlist can't do
    // (its raw text is deliberately excluded from the export). Without this,
    // imported EPG sources show up in Settings but the guide stays empty.
    if (state.epgSources.length) reloadEpgSources();
...
function bindEvents() {
  ...
  dom.exportEpgList?.addEventListener('click', exportEpgListFile);
